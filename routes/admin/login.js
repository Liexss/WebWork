...
const { searchuserHandler, selNameById, insertuserHandler } = require('../../models/db_mysqluser');

// 判断是否登录，身份认证
router.post('/', auth, async (req, res) => {
  res.send({ result: 1, msg: `欢迎，${req.currentUser}！` });
});
...
router.post('/verify', async (req, res) => {
  ...
  // 用户是否存在
  let doc = await searchuserHandler({ user_id });
  ...
  if (doc.result.length) {
    ...
    if (r !== doc.result[0].password) res.send({ result: 0, msg: '用户名或密码错误！' });
      ...
      // let user_name = await selNameById({ user_id });
