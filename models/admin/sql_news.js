...
// 更新文章时查重
const selupdateSql = async (vals, not) =>
  new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      err && console.log("连接失败");

      connection.query(
        `select * from news where ? and news_id != ${not}`,
        vals,
        (e, doc) => {
          e && console.log("查询失败");
          connection.release();
...
// 更新文章
const updatenewsSql = async (set, where) =>
  new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      err && console.log("连接失败");
...
// 删除文章
const deletenewsSql = async vals =>
  new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      err && console.log("连接失败");
